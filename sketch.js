//TREX GAme by LyzzyBeth using JS
...
function setup() {
  ...
  ground = createSprite(width / 2, height - 150, width, 4);
  ...
  score = 0;
  hiScore = 0;
  //indicator to check if highscore should be displayed or not in gamestate play
  displayHS = false;
...
function draw() {
  ...
  if (gameState == PLAY) {    
    ...
    //always display high score in gamestate end
    ...
    if (mousePressedOver(reset)) {
      //function call to start over the game after clicking on reset button
      startOver();
    }
  }
...
//function definition to start over the game after clicking on reset button
